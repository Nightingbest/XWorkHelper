...
// #define GET_comma_() comma = GetAsyncKeyState(VK_OEM_COMMA) & 0x8000//"<"的宏替换,旧的锁屏按键
...
int keyboard() // 检测按键,返回int值(1来回切屏;2暂停;3退出;4锁屏;未包含的预留功能(5左切屏;6右切屏))
{
    // 循环获取按键状态,由宏命令替换完成
    SHORT GET_win_();
    SHORT GET_cap_();
    SHORT GET_esc_();
    SHORT GET_back_();
    // SHORT GET_comma_();//旧的锁屏按键,检测"<"按键的状态,按下1,释放0
    SHORT GET_oem_2_();
    SHORT GET_period_();
    // SHORT GET_ctrl_(); // 左\右切屏的备选按键
    // SHORT GET_alt_();  // 左\右切屏的备选按键
    // 此处添加新按键
...
void function(int choice) // 接受功能代号,实现对应功能:已上线:0容错;1来回切屏;2暂停;3退出;4锁屏;未上线:5左切屏;6右切屏
{
    while (1)
    {
        ...
        case 4:
            ...
            break;
        case 5:
            qie_huan_left(); // 执行有容错的切换,但需要切换按键
            break;
        case 6:
            qie_huan_right(); // 执行有容错的切换,但需要切换按键
            break;
